...
import { Search, Plus, Trash2, ArrowDownCircle } from 'lucide-react'
...
export default function StockReceiveForm({ products }: { products: Product[] }) {
  ...
  function updateItem(index: number, field: 'receiveQty' | 'note', value: number | string) {
    const updated = [...items]
    if (field === 'receiveQty') updated[index].receiveQty = value as number
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">ข้อมูลการรับสินค้า</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">เลขที่อ้างอิง (PO/DO)</label>
            <input
              value={referenceNo}
              ...
              placeholder="เช่น PO-20240101-0001"
            />
          </div>
          <div>
      ...
      {/* Product Search */}
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">เลือกสินค้าที่รับเข้า</h2>
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 placeholder-gray-400"
              ...
              filteredProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!alreadyAdded && <Plus size={16} className="text-green-500" />}
                  </div>
                )
              })
            )}
        ...
        {items.length > 0 ? (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-center py-3 px-4 text-gray-500 font-medium w-36">จำนวนที่รับ</th>
                <th className="text-center py-3 px-4 text-gray-500 font-medium">คงเหลือหลังรับ</th>
                ...
                <tr key={item.productId} className="border-b border-gray-100">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center justify-center gap-2">
                      <button type="button"
                        onClick={() => updateItem(index, 'receiveQty', Math.max(1, item.receiveQty - 1))}
                        className="w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center font-bold text-gray-600"
                      >−</button>
                      <input
                        type="number" min={1} value={item.receiveQty}
                        onChange={(e) => updateItem(index, 'receiveQty', Math.max(1, Number(e.target.value)))}
                        className="w-14 text-center border border-gray-300 rounded-lg py-1 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                      />
                      <button type="button"
                        onClick={() => updateItem(index, 'receiveQty', item.receiveQty + 1)}
                        className="w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center font-bold text-gray-600"
                      >+</button>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-center font-bold text-green-600">
                    {item.currentQty + item.receiveQty} {item.unit}
                  </td>
                  <td className="py-3 px-4">
                    <input
                      value={item.note}
                      onChange={(e) => updateItem(index, 'note', e.target.value)}
                      className="w-full border border-gray-300 rounded-lg px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 placeholder-gray-400"
          ...
          <div className="text-center py-10 text-gray-400 border-2 border-dashed border-gray-200 rounded-xl">
            ...
            <p className="text-sm">ค้นหาและเพิ่มสินค้าที่ต้องการรับเข้า</p>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-3">
        <button type="submit" disabled={loading}
          className="flex items-center gap-2 bg-green-600 text-white px-8 py-2.5 rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 text-sm font-medium">
          <ArrowDownCircle size={18} />
          {loading ? 'กำลังบันทึก...' : 'ยืนยันรับสินค้าเข้า'}
        </button>
